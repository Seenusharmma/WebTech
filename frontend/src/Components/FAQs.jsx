      ...
      <div className="w-[89%] m-auto max-w-[800px] bg-gray-300 p-8 rounded-lg shadow-md">
        ...
        {questions.map((q) => (
          <div key={q.id} className="mb-4 last:mb-0">
            <button
              className={`w-full text-left text-2xl focus:outline-none p-4 rounded-lg shadow-md flex justify-between items-center transition-colors duration-300 
              ${
                activeQuestion === q.id
            ...
            <AnimatePresence>
              {activeQuestion === q.id && (
                <motion.div
                  ...
                  className="mt-2 text-gray-900 ml-4"
                >
                  <p>{q.answer}</p>
                </motion.div>
              )}
